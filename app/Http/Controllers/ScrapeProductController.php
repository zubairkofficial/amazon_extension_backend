...
class ScrapeProductController extends Controller
{
    ...
    protected function saveScrapeProduct($product, $code, $userId)
    {
        try {
            ...
            $scrapeproduct->imageUrls = isset($product['imageUrls']) ? $product['imageUrls'] : [];
    ...
    public function gptresponse($data, $userId)
    {
        foreach ($data as $id) {
            ...
            $content = $this->substituteValues($gptKey->prompt, $scrapeProduct, $systemProduct);
    ...
    public function testgptresponse()
    {
        $content = "explain web development";
        $gptKey = GptKey::first();
        $open_ai = new OpenAi("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");
