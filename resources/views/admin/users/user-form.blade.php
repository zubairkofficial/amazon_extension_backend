...
@section('content')
<h1 class="mb-2">{{ $formType === 'add' ? 'Create' : 'Update' }} User</h1>

<div class="text-end">
    <a href="/admin/users" class="btn btn-primary">Show All</a>
</div>

<form method="POST" action="{{ $formType === 'update' ? " /admin/users/{$user->id}" : '/admin/users' }}">
    @csrf
    @if($formType === 'update')
    @method('PUT')
    @endif
    <div class="form-group mb-3">
        ...
        <input type="text" class="form-control" id="id" name="id" value="{{ old('id') ?? $user->id ?? '' }}"
            autocomplete="off" {{ $formType==='update' ? 'readonly' : '' }}>
        @error('id')
